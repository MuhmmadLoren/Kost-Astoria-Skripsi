    ...
    <div class="card">
      <div class="card-content">
        <div class="card-body ">
          ...
          <p class="mt-1"> <i class="feather icon-phone-call"></i> <a href="tel:+62{{$kamar->user->no_wa}}"> {{$kamar->user->no_wa}}</a>  </p>

          <p class="mt-2" style="font-size: 12px">Hubungi pemilik kos untuk menanyakan lebih detail terkait kamar ini.</p>
<a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $kamar->user->no_wa) }}" target="_blank">
    <button class="btn btn-outline-black">Kirim pesan ke pemilik kos</button>
</a>

          <p class="mt-2" style="font-weight: bold; font-size:18px; color:black">Alamat:</p>
          <p class="mt-1" style="text-decoration: underline">Jl. Ki Ageng Kebo No.17, Kanigoro, Kec. Kartoharjo, Kota Madiun, Jawa Timur 63118</p>
          <hr>
          @if ($kamar->promo != null && $kamar->promo->end_date_promo >= Carbon\carbon::now()->format('d F, Y') && $kamar->promo->status == 1 )
            <p class="mt-2" style="font-weight: bold; font-size:18px; color:black">PROMO TERBATASS!</p>
