...
public function showkamar($slug)
{
    $kamar = kamar::with(['promo' => function ($q) {
        ...
        ->where('slug', $slug)
        ->firstOrFail(); // gunakan firstOrFail agar aman jika tidak ditemukan

    $fav = SimpanKamar::where('kamar_id', $kamar->id)
    ...
    public function showAllKamar(Request $request)
    {
        ...
        return view('front.allCardContent', \compact('allKamar', 'select', 'cari'));
    }
